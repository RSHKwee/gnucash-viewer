...
/**
 * (c) 2007 by <a href="http://Wolschon.biz> Wolschon Softwaredesign und
 * Beratung</a>.<br/>
...
public class TaxReportPanel extends JPanel {
  ...
  /**
   * Button in the {@link #myExportPanel} to export a CSV.
   */
  ...
  /**
   * Combobox to select a yearly, monthly or dayly export with the
   * {@link #myExportButton}.
   */
  ...
  private void initializeUI(final GnucashFile books) {
    ...
    final InputStream resourceAsStream = Thread.currentThread().getContextClassLoader()
        .getResourceAsStream("TaxReportPanel.xml");
    ...
    if (resourceAsStream != null) {
      ...
      /*
       * try { props.loadFromXML(getClass().getResourceAsStream(
       * "TaxReportPanel.xml")); props.storeToXML(new FileOutputStream(configFile),
       ...
       * + "sum.([0-9]*).type = ONLYTO - sum only the ones that increase the balance
       * of the account (other values: ONLYFROM, ALL)\n" + "sum.([0-9]*).type =
       * ONLYFROM - sum only the ones that decrease the balance of the account (other
       * values: ONLYFROM, ALL)\n" + "sum.([0-9]*).type = ALL - sum all the ones that
       * increase or decreas the balance of the account (other values: ONLYFROM,
       * ALL)\n" +
       ...
       * configFile.getAbsolutePath()); } catch (Exception e) {
       * LOGGER.error("Problem loading or storing default-TaxReportPanel.xml", e);
       * JLabel errorLabel = new JLabel(e.getMessage()); this.add(errorLabel,
       * BorderLayout.CENTER); return; }
       */
    }
  ...
  /**
   * Show a dialog to export a CSV-file that contains the shown
   * {@link TransactionSum}s for each month, year or day.
   */
...
//		}
  ...
  private LocalDate getMinDate() {
    ...
    return null;/*
                 * new Date ((new GregorianCalendar(1970, 01, 01)) .getTimeInMillis())
                 */
  }
  ...
  private LocalDate getMaxDate() {
    // TODO: provide an input-field for the year.
    return null/*
                * new Date ((new GregorianCalendar(2100, 12, 31) ).getTimeInMillis())
                */;
  }
