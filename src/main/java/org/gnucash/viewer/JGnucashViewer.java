...
public class JGnucashViewer extends JFrame {
  ...
  /**
   * (c) 2010 by <a href="http://Wolschon.biz>Wolschon Softwaredesign und
   * Beratung</a>.<br/>
   ...
   * <br/>
   * <br/>
   * <b>Wrapper for an {@link AccountAction} that knows about
   * {@link JGnucashViewer#getSelectedAccount()}.</b>
   *
  ...
  /**
   * The {@link JTabbedPane} containing {@link #transactionsPanel} and
   * {@link #taxReportPanel}.
   */
  ...
  /**
   * This method initializes the GnucashViewer.
   */
      ...
      accountsTree.addTreeSelectionListener(new TreeSelectionListener() {
        public void valueChanged(final TreeSelectionEvent e) {
          ...
          if (path == null) {
            ...
            GnucashAccountsTreeModel.GnucashAccountTreeEntry entry = (GnucashAccountsTreeModel.GnucashAccountTreeEntry) path
  ...
  public void setSelectedAccount(final GnucashAccount aSelectedAccount) {
    ...
    if (selectedAccount != null) {
      LOGGER.debug("accoun " + selectedAccount.getId() + " = " + selectedAccount.getQualifiedName() + " selected");
    }
  }
  ...
  private javax.swing.JPanel getJContentPane() {
    if (jContentPane == null) {
      ...
      jContentPane.setBorder(javax.swing.BorderFactory.createEmptyBorder(border, border, border, border));
  ...
  /**
   * This method initializes jFileChooser. If is used for the open-dialog. In
   * JGnuCash it is also used for the save, save as and import -dialog.
   *
  ...
  protected javax.swing.JFileChooser getJFileChooser() {
    if (jFileChooser == null) {
      if (!m_param.get_InputFile().isBlank()) {
        jFileChooser = new javax.swing.JFileChooser(m_param.get_InputFile());
      } else {
  ...
  protected boolean loadFile() {
    ...
    if (state == JFileChooser.APPROVE_OPTION) {
      ...
      if (!f.exists()) {
        JOptionPane.showMessageDialog(JGnucashViewer.this, "File does not exist", "missing file",
  ...
  public void setModel(final GnucashFile model) {
    if (model == null) {
      throw new IllegalArgumentException("null not allowed for field this.model");
    }
    myModel = model;
  ...
  protected JPopupMenu getAccountTreePopupMenu() {
    if (myAccountTreePopupMenu == null) {
      ...
      LOGGER.debug(
          "getAccountTreePopupMenu() created menu with " + myAccountTreePopupMenu.getComponentCount() + " entries");
  ...
  public static void main(final String[] args) {
    ...
    if (args.length > 0) {
      ...
      if (!m_param.get_InputFile().isBlank()) {
        ste.loadFile(new File(m_param.get_InputFile()));
      }
    }
